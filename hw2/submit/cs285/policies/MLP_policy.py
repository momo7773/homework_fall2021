...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        action = None
        dis = self(ptu.from_numpy(observation))
        action = ptu.to_numpy(dis.sample())
        # if self.discrete:
        #   action = ptu.to_numpy(dis.sample())
        # else:
        #   action = ptu.to_numpy(dis.rsample())
        return action
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        advantages = ptu.from_numpy(np.asarray(advantages))
        ...
        loss = torch.sum(torch.mul(distribution.log_prob(actions),advantages))*-1 / advantages.shape[0]
        ...
        if self.nn_baseline: 
            ...
            # TODO
          v_predict = self.baseline(observations).reshape(-1,1)
          q_values = q_values.reshape(-1,1)
          q_values_normalized = ptu.from_numpy(utils.normalize(q_values, np.mean(q_values),np.std(q_values)))
          
          loss_baseline = self.baseline_loss(v_predict, q_values_normalized)
