...
class PGAgent(BaseAgent):
    def __init__(self, env, agent_params):
        ...
        self.gamma = self.agent_params['gamma'] #discount_rate
        self.standardize_advantages = self.agent_params['standardize_advantages']
        self.nn_baseline = self.agent_params['nn_baseline']
        self.reward_to_go = self.agent_params['reward_to_go']
        self.gae_lambda = self.agent_params['gae_lambda']#gae hyperparameter
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT 2: self.gae_lambda is the lambda value in the
                        ## GAE formula
                    v_t_1 = 0
                    if terminals[i] != 1 and i < batch_size - 1:
                      v_t_1 = values[i+1]
                    v_t = values[i]
                    delta_t = rews[i] + self.gamma * v_t_1 - v_t
                ...
                #Todo: not sure whether normalized q_values
    ...
    def _discounted_return(self, rewards):
        ...
        reward_total_discount = 0
        cnt = 0
        for reward in rewards:
          reward_total_discount += (self.gamma ** cnt) * reward
          cnt += 1
        
        list_of_discounted_returns = [reward_total_discount for _ in range(len(rewards))]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        cnt = 0
        for reward in rewards:
          rewards_initial += (self.gamma ** cnt) * reward
          cnt += 1
        list_of_discounted_cumsums = [rewards_initial]
        for i in range(1,len(rewards)):
          reward_new = (list_of_discounted_cumsums[i-1] - rewards[i-1]) / self.gamma
          list_of_discounted_cumsums.append(reward_new)
